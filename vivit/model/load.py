...
def load_model(
    ...
    """
    Loads the vivit model and image processor.
    ...
    Args:
        ...
        freeze_base (bool): If True, freeze the base 'vivit' layers except last `unfreeze_last_n`.
        unfreeze_last_n (int): Number of last 'vivit' encoder layers to unfreeze (only relevant if freeze_base=True).

    Returns:
        model (VivitForVideoClassification): The loaded model ready for training or inference.
    """
    ...
    if not freeze_base:
        ...
        # Primero congelamos todo el backbone vivit
        for name, param in model.named_parameters():
            if name.startswith("vivit."):
